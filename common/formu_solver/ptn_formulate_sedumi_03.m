function [formulate_form_sedumi] = ptn_formulate_sedumi_03(ptn_info, pen_digit_case)
...
MatrixA = sparse(ptn_info.N, ptn_info.dim_feature * ptn_info.dim_feature + ptn_info.N + ptn_info.dim_feature + 1);
%%% Formulate the constraints
for ii = 1:1:ptn_info.N
    for jj = 1:1:ptn_info.dim_feature
        MatrixA(ii, jj) = pen_digit_case(ii).feature(jj)/ptn_info.scale_feature_factor;
    end
    ...
    psd_variable_list = ptn_info.dim_feature + 1 + ptn_info.N + 1;
    for jj = 1:1:ptn_info.dim_feature
    ...
    MatrixA(ii, ii+ ptn_info.dim_feature + 1) = 1;   % variable indicating the error distance between the model output and the class_id
    RHS(ii, 1) = pen_digit_case(ii).class * ptn_info.scale_factor + ptn_info.scale_offset;
end

%%% The cost vector
c = sparse([zeros(ptn_info.dim_feature, 1);1; zeros(ptn_info.N + ptn_info.dim_feature * ptn_info.dim_feature, 1)]);

%%% First ptn_info.dim_feature variables are free
K.f = ptn_info.dim_feature;
