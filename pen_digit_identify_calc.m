function stOutPenDigitIdentifyCalc = pen_digit_identify_calc(ptn_info, astSampleCase, stPenDataNeuralNetwork)

flag_load_data = ptn_info.flag_load_data;
stOutPenDigitIdentifyCalc = [];
netSolution = [];
astSampleCase_pca = astSampleCase;
ptn_cluster_solution = [];
class_model = [];
formulate_form_sedumi = [];
...
elseif ptn_info.model_option == 11              %%%%%%%% Neural network
    ...
    net.trainParam.epochs = ptn_info.nNeuralMaxIteration;
    net.trainParam.mu = ptn_info.fNeuralLearnRate;
    N = ptn_info.N;
    matTrainPoints = stPenDataNeuralNetwork.matPoints(:, 1:N);
    matTargetClass = stPenDataNeuralNetwork.matTargetClass(:, 1:N);
...
stOutPenDigitIdentifyCalc.cpu_time_learning = cpu_time_learning;
stOutPenDigitIdentifyCalc.netSolution = netSolution;
...
stOutPenDigitIdentifyCalc.model_linear = model_linear;
stOutPenDigitIdentifyCalc.Model_Ord2 = Model_Ord2;
...
elseif ptn_info.model_option == 11
    t1 = cputime;
    [pattern_identify, remain_error] = ptn_verify_model_neural_network(netSolution, astSampleCase, ptn_info, stPenDataNeuralNetwork);
    identify_time = cputime - t1
...
stOutPenDigitIdentifyCalc.astSampleCase = astSampleCase;
stOutPenDigitIdentifyCalc.identify_time = identify_time;
stOutPenDigitIdentifyCalc.formulate_form_sedumi = formulate_form_sedumi;
stOutPenDigitIdentifyCalc.remain_error = remain_error;
stOutPenDigitIdentifyCalc.pattern_identify = pattern_identify;
stOutPenDigitIdentifyCalc.astSampleCase_pca = astSampleCase_pca;
stOutPenDigitIdentifyCalc.ptn_cluster_solution = ptn_cluster_solution;
stOutPenDigitIdentifyCalc.class_model = class_model ;
